...
class ModelingTrainer(Trainer):
    ...
    def _run_evaluate_end(self, metrics: dict, eavesdrop=False):
        if not eavesdrop: self._run_scheduler(metrics)
    ...
    def _run_epoch(self):
        ...
        with tqdm(enumerate(self._trainloader), total=num_batch,
            ...
            for step, (inputs, target) in pbar:
                ...
                indices = (target >= 2).nonzero(as_tuple=True)[0]

                outputs, nce = self._model(inputs, target_indices=indices)
                ...
                # contrastive learning
                if nce != 0: loss = loss + nce
                # if nce != 0: loss = 0.9*loss + 0.5*nce
    ...
    def predict(
        ...
        left_vocab_i2v = self._model.pipeline.prd_text_field.vocab.i2v
        right_vocab_i2v = self._model.pipeline.rvw_text_field.vocab.i2v
        left_vocab_v2i = self._model.pipeline.prd_text_field.vocab.v2i
        right_vocab_v2i = self._model.pipeline.rvw_text_field.vocab.v2i
        ...
        with torch.no_grad():
            ...
            for inputs, labels in tqdm(dataloader):
                ...
                outputs = self._model(inputs)[0].detach().cpu()

                # experiments
                unhelpful_labels = (labels <= 1)
                helpful_outputs = (outputs >= 3)
                target_indices = (unhelpful_labels & helpful_outputs).nonzero(as_tuple=True)[0]
                random_idx = 23
                # random_idx = target_indices[random.randint(0, target_indices.size(0))].item()
                label = labels[random_idx]
                output = outputs[random_idx]
                id_text_left = inputs['text_left'][random_idx].cpu()
                id_text_left = id_text_left[id_text_left != 0].tolist()
                id_text_right = inputs['text_right'][random_idx].cpu()
                id_text_right = id_text_right[id_text_right != 0].tolist()

                token_left = [left_vocab_i2v[id] for id in id_text_left]
                token_right = [right_vocab_i2v[id] for id in id_text_right]
                text_left = ' '.join(token_left)
                text_right = ' '.join(token_right)
                text_right += " kinwatch jam tangan q & q"
                token_right = text_right.split()
                text_right_length = inputs['text_right_length'][random_idx].cpu().item()
                id_text_right = [right_vocab_v2i[word] for word in token_right]
                id_text_right = torch.tensor(id_text_right).cuda()
                
                batch = {}
                batch['text_left'] = inputs['text_left'][random_idx].unsqueeze(0)
                batch['text_right'] = id_text_right.unsqueeze(0) 
                batch['text_left_length'] = inputs['text_left_length'][random_idx].unsqueeze(0)
                batch['text_right_length'] = torch.tensor(len(id_text_right)).unsqueeze(0).cuda()

                batch['image_left'] = inputs['image_left'][random_idx].unsqueeze(0)
                batch['image_right'] = inputs['image_right'][random_idx].unsqueeze(0)
                batch['image_left_length'] = inputs['image_left_length'][random_idx].unsqueeze(0)
                batch['image_right_length'] = inputs['image_right_length'][random_idx].unsqueeze(0)
                import pdb
                pdb.set_trace()
                new_output = self._model(batch)
